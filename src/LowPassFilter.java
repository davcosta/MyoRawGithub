...
public class LowPassFilter {
	...
	double[] filterSignal(CopyOnWriteArrayList<Double> emg1, double sampleRate ,double cutoffFreq, double filterOrder, int filterType, double ripplePercent) {
        double[][] recursionCoefficients =   new double[22][2];
        // Generate double array for ease of coding
        double[] unfilteredSignal =   new double[emg1.size()];
        for (int i=0; i<emg1.size(); i++) {
            unfilteredSignal[i] =   emg1.get(i);
        }
        ...
        double[] filteredSignal =   new double[emg1.size()];
        double filterSampleA    =   0;
        double filterSampleB    =   0;

        // loop for applying recursive filter 
        for (int i= (int) Math.round(filterOrder); i<emg1.size(); i++){
